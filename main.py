...
def global_exception_handler(exc_type, exc_value, exc_traceback):
    ...
    logger.error("异常堆栈:", exc_info=(exc_type, exc_value, exc_traceback))
    
    # 尝试优雅关闭
    try:
        global my_handle
        if my_handle is not None:
            logger.info("尝试优雅关闭系统...")
            my_handle.shutdown()
...
def validate_danmaku_message(msg: dict) -> bool:
    """验证弹幕消息格式"""
    if not isinstance(msg, dict):
        return False
    
    # 检查必需字段
    required_fields = ['type', 'platform', 'username', 'content']
    ...
    if not isinstance(msg.get('type'), str):
        return False
    if not isinstance(msg.get('platform'), str):
        return False
    if not isinstance(msg.get('username'), str):
        return False
    if not isinstance(msg.get('content'), str):
        return False
    
    # 验证消息类型
    valid_types = ['comment', 'gift', 'entrance', 'follow', 'like', 'super_chat']
...
def start_server():
    ...
    try:
        ...
        if my_handle is None:
            ...
            raise RuntimeError("My_handle初始化返回None")
    except Exception as e:
        logger.error(f"程序初始化失败: {e}")
        logger.error(traceback.format_exc())
        # 尝试清理资源
        try:
            if 'my_handle' in locals() and my_handle is not None:
                my_handle.shutdown()
        except Exception as cleanup_e:
            logger.error(f"清理资源失败: {cleanup_e}")
        os._exit(1)
    ...
    try:
        if config.get("live2d", "enable"):
            ...
            logger.info(f"启动Live2D服务器，端口: {web_server_port}")
            
            def live2d_thread_wrapper():
                try:
                    asyncio.run(web_server_thread(web_server_port))
                except Exception as e:
                    logger.error(f"Live2D服务器运行异常: {e}")
                    logger.error(traceback.format_exc())
            
            live2d_thread = threading.Thread(target=live2d_thread_wrapper, daemon=True)
            live2d_thread.start()
            logger.info("Live2D线程启动成功")
    except Exception as e:
        logger.error(f"Live2D线程启动失败: {e}")
        logger.error(traceback.format_exc())
        # Live2D失败不应该导致整个程序退出
        logger.warning("Live2D服务启动失败，程序将继续运行")
    ...
    if platform != "wxlive":
        ...
        def http_api_thread():
            ...
            from utils.models import (
                ...
                CallbackMessage,
                CommonResult,
                DanmakuManageMessage,
                DanmakuConnectionStatus,
            )
            ...
            async def send(msg: SendMessage):
                ...
                try:
                    ...
                    if data_json["type"] in ["reread", "reread_top_priority"]:
                        ...
                        # 恢复HTTP API弹幕处理功能，使用三层队列机制
                        logger.info(f"HTTP API接收到弹幕数据：{data_json}")
                        # 使用三层队列处理策略处理弹幕
            ...
            """
            接收danmaku_server推送的弹幕数据
            这是AI-Vtuber接收弹幕的主要入口
            """
            @app.post('/api/danmaku/receive')
            async def receive_danmaku_message(request: Request = None, msg: dict = None):
                try:
                    # 支持两种调用方式：FastAPI Request对象或直接传入dict
                    if request is not None:
                        msg = await request.json()
                    elif msg is None:
                        return CommonResult(code=400, message="缺少消息数据")
                    
                    logger.info(f'收到弹幕数据：{msg}')
                    
                    # 验证消息格式
                    if not validate_danmaku_message(msg):
                        logger.warning(f'弹幕消息格式不正确：{msg}')
                        return CommonResult(code=400, message="消息格式不正确", data={"required_fields": ["type", "platform", "username", "content"]})
                    
                    # 将弹幕数据放入处理队列
                    success = await my_handle.process_danmaku_message(msg)
                    
                    if success:
                        return CommonResult(code=200, message="弹幕数据接收成功", data={"status": "processed"})
                    else:
                        return CommonResult(code=-1, message="弹幕数据处理失败", data={"status": "failed"})
                        
                except json.JSONDecodeError as e:
                    logger.error(f"JSON解析失败：{e}")
                    return CommonResult(code=400, message="JSON格式错误")
                except Exception as e:
                    logger.error(f"接收弹幕数据失败：{traceback.format_exc()}")
                    return CommonResult(code=-1, message=f"接收弹幕数据失败！{e}")
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"获取弹幕连接状态失败！{e}")

            """
            获取多平台弹幕统计信息
            """
            @app.get('/api/danmaku/stats')
            async def get_danmaku_stats():
                try:
                    logger.info('获取多平台弹幕统计信息')
                    stats = my_handle.get_multi_platform_stats()
                    return CommonResult(code=200, message="获取成功", data=stats)
                except Exception as e:
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"获取弹幕统计信息失败！{e}")

            """
            获取弹幕WebSocket客户端状态
            """
            @app.get('/api/danmaku/websocket-status')
            async def get_danmaku_websocket_status():
                try:
                    logger.info('获取弹幕WebSocket客户端状态')
                    status = my_handle.get_danmaku_websocket_status()
                    return CommonResult(code=200, message="获取成功", data=status)
                except Exception as e:
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"获取弹幕WebSocket客户端状态失败！{e}")

            """
            获取弹幕接收状态和配置信息
            """
            @app.get('/api/danmaku/receive-status')
            async def get_danmaku_receive_status():
                try:
                    logger.info('获取弹幕接收状态')
                    
                    # 获取队列状态
                    queue_size = my_handle.danmaku_output_queue.qsize() if hasattr(my_handle, 'danmaku_output_queue') else 0
                    
                    status_data = {
                        "api_enabled": True,
                        "queue_size": queue_size,
                        "supported_message_types": ["comment", "gift", "entrance", "follow", "like", "super_chat"],
                        "required_fields": ["type", "platform", "username", "content"],
                        "endpoint": "/api/danmaku/receive",
                        "method": "POST"
                    }
                    
                    return CommonResult(code=200, message="获取成功", data=status_data)
                except Exception as e:
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"获取弹幕接收状态失败！{e}")

            """
            获取多平台配置
            """
            @app.get('/api/danmaku/multi-platform-config')
            async def get_multi_platform_config():
                try:
                    logger.info('获取多平台配置')
                    config_data = Config(config_path).get("danmaku", "multi_platform", {})
                    return CommonResult(code=200, message="获取成功", data=config_data)
                except Exception as e:
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"获取多平台配置失败！{e}")

            """
            更新多平台配置
            """
            @app.post('/api/danmaku/multi-platform-config')
            async def update_multi_platform_config(request: Request):
                try:
                    data_json = await request.json()
                    logger.info(f'更新多平台配置：{data_json}')
                    
                    # 更新配置文件
                    config_obj = Config(config_path)
                    config_obj.set("danmaku", "multi_platform", data_json)
                    config_obj.save(config_path)
                    
                    # 通知my_handle更新配置
                    if hasattr(my_handle, 'update_multi_platform_config'):
                        my_handle.update_multi_platform_config(data_json)
                    
                    return CommonResult(code=200, message="配置更新成功")
                except Exception as e:
                    logger.error(traceback.format_exc())
                    return CommonResult(code=-1, message=f"更新多平台配置失败！{e}")
                ...
                try:
                    ...
                    if data_json["type"] in ["audio_playback_completed"]:
                        ...
                        if data_json["data"]["wait_play_audio_num"] > int(
                            ...
                            logger.info(
                                f'等待播放的音频数量大于限定值，闲时任务的闲时计时由 {my_global.global_idle_time} -> {int(config.get("idle_time_task_config", "idle_time_reduce_to"))}秒'
                            )
                            # 闲时任务的闲时计时 清零
                            my_global.global_idle_time = int(
                                config.get("idle_time_task_config", "idle_time_reduce_to")
                            )
            ...
            # 启动弹幕WebSocket服务器（在uvicorn.run之前）
            if hasattr(my_handle, '_need_start_websocket_server') and my_handle._need_start_websocket_server:
                import asyncio
                import threading
                
                def run_websocket_server():
                    """WebSocket服务器运行函数，包含异常处理"""
                    try:
                        loop = asyncio.new_event_loop()
                        asyncio.set_event_loop(loop)
                        logger.info("启动弹幕WebSocket服务器...")
                        loop.run_until_complete(my_handle.start_danmaku_websocket_server())
                        # 保持事件循环运行
                        loop.run_forever()
                    except Exception as e:
                        ...
                        try:
                            ...
                            logger.error(f"关闭WebSocket事件循环失败: {e}")
                
                # 在后台线程中启动WebSocket服务器
                try:
                    websocket_thread = threading.Thread(target=run_websocket_server, daemon=True)
                    websocket_thread.start()
                    my_handle._need_start_websocket_server = False
                    logger.info("WebSocket服务器线程已启动")
                except Exception as e:
                    logger.error(f"启动WebSocket服务器线程失败: {e}")
                    logger.error(traceback.format_exc())
                ...
                if config.get("webui", "local_dir_to_endpoint", "enable"):
                    for tmp in config.get("webui", "local_dir_to_endpoint", "config"):
                        try:
                            ...
                            logger.info(f"挂载静态文件目录: {tmp['local_dir']} -> {tmp['url_path']}")
                        except Exception as e:
                            logger.error(f"挂载静态文件目录失败 {tmp['local_dir']}: {e}")
            except Exception as e:
                logger.error(f"配置静态文件目录失败: {e}")
                    
            try:
                api_port = config.get("api_port")
                logger.info(f"启动uvicorn服务器，端口: {api_port}")
                uvicorn.run(app, host="0.0.0.0", port=api_port, log_level="info")
                #uvicorn.run(app, host="0.0.0.0", port=api_port, ssl_certfile="F:\\FunASR_WS\\cert.pem", ssl_keyfile="F:\\FunASR_WS\\key.pem")
            except Exception as e:
        ...
        # HTTP API线程并启动
        def http_api_thread_wrapper():
            """HTTP API线程包装器，添加异常处理和重试机制"""
            max_retries = 3
            retry_delay = 5
            
            for attempt in range(max_retries):
                try:
                    logger.info(f"启动HTTP API服务器 (尝试 {attempt + 1}/{max_retries})")
                    http_api_thread()
                    break  # 如果成功启动，跳出重试循环
                except Exception as e:
                    logger.error(f"HTTP API服务器启动失败 (尝试 {attempt + 1}/{max_retries}): {e}")
                    logger.error(traceback.format_exc())
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.error("HTTP API服务器启动失败，已达到最大重试次数")
                        # 不退出整个程序，只记录错误
                        break
        
        try:
            inside_http_api_thread = threading.Thread(target=http_api_thread_wrapper, daemon=True)
            inside_http_api_thread.start()
            logger.info("HTTP API线程已启动")
        except Exception as e:
            logger.error(f"启动HTTP API线程失败: {e}")
            logger.error(traceback.format_exc())
    ...
    def record_audio():
        ...
        try:
            ...
            if pressdown_num >= 5:  # 粗糙的处理手段
                logger.info(f"录音完成，时长: {pressdown_num} 帧")
                return 1
            else:
            ...
            if not is_recording:
                try:
                    ...
                    if config.get("talk", "continuous_talk"):
                        ...
                        do_listen_and_comment_thread.daemon = True
                        do_listen_and_comment_thread.start()
                        logger.info("连续对话线程已启动")
                    else:
                        stop_do_listen_and_comment_thread_event.clear()
                        do_listen_and_comment_thread = threading.Thread(
                        ...
                        do_listen_and_comment_thread.daemon = True
                        do_listen_and_comment_thread.start()
                        logger.info("单次对话线程已启动")
                except Exception as e:
                    logger.error(f"启动语音对话线程失败: {e}")
                    logger.error(traceback.format_exc())
            else:
                logger.warning("正在录音中...请勿重复点击录音捏！")
    
        logger.error(f"按键事件处理异常: {e}")
        logger.error(traceback.format_exc())

    # 按键监听
    def key_listener():
        try:
            logger.info("启动按键监听服务...")
            # 注册按键按下事件的回调函数
            ...
            logger.info("按键监听服务被用户中断")
            os._exit(0)
        except Exception as e:
            logger.error(f"按键监听服务异常: {e}")
            logger.error(traceback.format_exc())
            # 尝试重启按键监听
            logger.info("尝试重启按键监听服务...")
            time.sleep(2)
            key_listener()
    ...
    try:
        ...
        thread.daemon = True
        thread.start()
        logger.info("按键监听线程已启动")
    except Exception as e:
        logger.error(f"启动按键监听线程失败: {e}")
        logger.error(traceback.format_exc())
    ...
    def run_schedule():
        ...
        while True:
            ...
            # 使用更长的间隔减少频繁检查
            time.sleep(3)  # 控制每次循环的间隔时间，避免过多占用 CPU 资源
    ...
    if config.get("trends_copywriting", "enable"):
        try:
            # 创建动态文案子线程并启动
            trends_thread = threading.Thread(target=lambda: asyncio.run(run_trends_copywriting()))
            trends_thread.daemon = True
            trends_thread.start()
            logger.info("动态文案线程已启动")
        except Exception as e:
            logger.error(f"启动动态文案线程失败: {e}")
            logger.error(traceback.format_exc())
    ...
    async def idle_time_task():
        ...
        try:
            if not config.get("idle_time_task_config", "enable"):
                return
            ...
            overflow_time_min = int(config.get("idle_time_task_config", "idle_time_min"))
            overflow_time_max = int(config.get("idle_time_task_config", "idle_time_max"))
            overflow_time = random.randint(overflow_time_min, overflow_time_max)
            
            # 获取闲时任务的其他参数
            wait_play_audio_num_threshold = int(config.get("idle_time_task_config", "wait_play_audio_num_threshold"))
            idle_time_reduce_to = int(config.get("idle_time_task_config", "idle_time_reduce_to"))

            logger.info(f"下一个闲时任务将在{overflow_time}秒后执行")
            logger.info(f"闲时任务参数: min={overflow_time_min}s, max={overflow_time_max}s, 音频队列阈值={wait_play_audio_num_threshold}, 减少到={idle_time_reduce_to}s")

            def load_data_list(type):
                if type == "copywriting":
                    tmp = config.get("idle_time_task_config", "copywriting", "copy")
                elif type == "comment":
            ...
            def calculate_next_overflow_time():
                """计算下一个闲时任务的等待时间，考虑音频队列阈值和减少时间设置"""
                nonlocal overflow_time
                
                # 检查是否需要减少等待时间
                if (wait_play_audio_num_threshold > 0 and 
                    idle_time_reduce_to > 0 and 
                    my_global.wait_play_audio_num < wait_play_audio_num_threshold and 
                    overflow_time > idle_time_reduce_to):
                    
                    logger.info(f"音频队列数量({my_global.wait_play_audio_num})小于阈值({wait_play_audio_num_threshold})，将等待时间从{overflow_time}秒减少到{idle_time_reduce_to}秒")
                    overflow_time = idle_time_reduce_to
                else:
                    # 正常随机生成等待时间
                    overflow_time = random.randint(overflow_time_min, overflow_time_max)
            ...
            def do_task(
                ...
                logger.debug(f"执行闲时任务，当前模式: {last_mode}")

                # 闲时任务处理
                if config.get("idle_time_task_config", "copywriting", "enable"):
                    if last_mode == 0:
                        # 是否开启了随机触发
                        if config.get("idle_time_task_config", "copywriting", "random"):
            ...
            while True:
                ...
                if overflow_time_min > 0 and overflow_time_max > 0:
                    # 每隔一秒的睡眠进行闲时计数，优化性能减少频繁检查
                    await asyncio.sleep(1)
                else:
                    # 优化性能：增加最小睡眠时间，减少CPU占用
                    await asyncio.sleep(0.5)
                my_global.global_idle_time = my_global.global_idle_time + 1

                # 检查是否需要减少等待时间（即使没有执行任务）
                if (wait_play_audio_num_threshold > 0 and 
                    idle_time_reduce_to > 0 and 
                    my_global.wait_play_audio_num < wait_play_audio_num_threshold and 
                    overflow_time > idle_time_reduce_to):
                    ...
                    # 如果当前闲时计时已经超过了减少后的时间，则重置计时器以便立即在下次检查时触发
                    if my_global.global_idle_time >= idle_time_reduce_to:
                        logger.info(f"当前闲时计时({my_global.global_idle_time}秒)已达到减少后的等待时间({idle_time_reduce_to}秒)，重置计时器以便立即触发")
                        my_global.global_idle_time = idle_time_reduce_to

                logger.debug(f"闲时计数: {my_global.global_idle_time}/{overflow_time}, 音频队列数量: {my_global.wait_play_audio_num}")

                if config.get("idle_time_task_config", "type") == "直播间无消息更新闲时":
                    # 闲时计数达到指定值，进行闲时任务处理
                    if my_global.global_idle_time >= overflow_time:
                        logger.info(f"闲时计数达到{overflow_time}秒，触发闲时任务")
                        (
                            last_mode,
                            copywriting_copy_list,
                            comment_copy_list,
                        ...
                        # 更新overflow_time为下一个任务的等待时间
                        overflow_time = calculate_next_overflow_time()
                        logger.info(f"闲时任务执行完成，下一个任务将在{overflow_time}秒后执行")
                elif config.get("idle_time_task_config", "type") == "待合成消息队列更新闲时":
                    if (my_global.global_idle_time >= overflow_time and 
                        my_handle.is_queue_less_or_greater_than(
                            type="message",
                            less=int(
                                config.get("idle_time_task_config", "min_msg_queue_len_to_trigger")
                            ),
                        )):
                        logger.info("待合成消息队列数量不足，触发闲时任务")
                        (
                            last_mode,
                            copywriting_copy_list,
                            comment_copy_list,
                        ...
                        overflow_time = calculate_next_overflow_time()
                        logger.info(f"闲时任务执行完成，下一个任务将在{overflow_time}秒后执行")
                elif config.get("idle_time_task_config", "type") == "待播放音频队列更新闲时":
                    ...
                    if config.get("visual_body") == "metahuman_stream":
                        if (my_global.global_idle_time >= overflow_time and 
                            my_global.wait_play_audio_num < config.get("idle_time_task_config", "min_audio_queue_len_to_trigger")):
                            logger.info("metahuman_stream平台音频队列数量不足，触发闲时任务")
                            (
                                last_mode,
                                copywriting_copy_list,
                                comment_copy_list,
                        ...
                        if (my_global.global_idle_time >= overflow_time and 
                            my_handle.is_queue_less_or_greater_than(
                                ...
                                less=int(
                                    config.get(
                                        "idle_time_task_config", "min_audio_queue_len_to_trigger"
                                    )
                                ),
                            )):
                            logger.info("待播放音频队列数量不足，触发闲时任务")
                            (
                                last_mode,
                                copywriting_copy_list,
                                comment_copy_list,
            ...
            logger.error(f"闲时任务执行异常: {e}")
            logger.error(traceback.format_exc())

    # 闲时任务包装函数，添加异常处理和重试机制
    def idle_time_task_wrapper():
        max_retries = 3
        retry_count = 0
        ...
        while retry_count < max_retries:
            try:
                logger.info("启动闲时任务...")
                asyncio.run(idle_time_task())
                break  # 正常完成，退出重试循环
            except Exception as e:
                retry_count += 1
                logger.error(f"闲时任务异常 (重试 {retry_count}/{max_retries}): {e}")
                logger.error(traceback.format_exc())
                if retry_count < max_retries:
                    logger.info(f"5秒后重试闲时任务...")
                    time.sleep(5)
                else:
                    logger.error("闲时任务重试次数已达上限，停止重试")

    if config.get("idle_time_task_config", "enable"):
        try:
            # 创建闲时任务子线程并启动
            idle_task_thread = threading.Thread(target=idle_time_task_wrapper)
            idle_task_thread.daemon = True  # 设置为守护线程
            idle_task_thread.start()
            logger.info("闲时任务线程已启动")
        except Exception as e:
            logger.error(f"启动闲时任务线程失败: {e}")
            logger.error(traceback.format_exc())
    ...
    def run_image_recognition_schedule(interval: int, type: str):
        ...
        try:
            logger.info(f"启动图像识别定时任务: {type}, 间隔: {interval}秒")
            schedule.every(interval).seconds.do(
                partial(image_recognition_schedule_task, type)
            )
        except Exception as e:
            logger.error(f"图像识别定时任务初始化失败: {e}")
            logger.error(traceback.format_exc())
            return
        ...
        while True:
            try:
                schedule.run_pending()
                # 使用更长的间隔减少频繁检查
                time.sleep(5)  # 控制每次循环的间隔时间，避免过多占用 CPU 资源
            except Exception as e:
                logger.error(f"图像识别定时任务执行异常: {e}")
                logger.error(traceback.format_exc())
                # 使用更长的间隔减少频繁检查
    ...
    if config.get("image_recognition", "loop_screenshot_enable"):
        try:
            ...
            image_recognition_schedule_thread.daemon = True
            image_recognition_schedule_thread.start()
            logger.info("窗口截图定时任务线程已启动")
        except Exception as e:
            logger.error(f"启动窗口截图定时任务失败: {e}")
            logger.error(traceback.format_exc())
    ...
    if config.get("image_recognition", "loop_cam_screenshot_enable"):
        try:
            ...
            image_recognition_cam_schedule_thread.daemon = True
            image_recognition_cam_schedule_thread.start()
            logger.info("摄像头截图定时任务线程已启动")
        except Exception as e:
            logger.error(f"启动摄像头截图定时任务失败: {e}")
            logger.error(traceback.format_exc())
    ...
    if config.get("visual_body") == "metahuman_stream":
        ...
        def run_metahuman_stream_is_speaking_schedule():
            interval = 3
            try:
                logger.info(f"启动LiveTalking状态检测定时任务，间隔: {interval}秒")
                schedule.every(interval).seconds.do(
                    partial(metahuman_stream_is_speaking)
                )
            except Exception as e:
                logger.error(f"LiveTalking状态检测定时任务初始化失败: {e}")
                logger.error(traceback.format_exc())
                return
            ...
            while True:
                try:
                    ...
                    logger.error(traceback.format_exc())
                    # 使用更长的间隔减少频繁检查
                    time.sleep(10)  # 异常后等待5秒再继续    

        try:
            run_metahuman_stream_is_speaking_schedule_thread = threading.Thread(
                target=run_metahuman_stream_is_speaking_schedule
            )
            run_metahuman_stream_is_speaking_schedule_thread.daemon = True
            run_metahuman_stream_is_speaking_schedule_thread.start()
            logger.info("LiveTalking状态检测定时任务线程已启动")
        except Exception as e:
            logger.error(f"启动LiveTalking状态检测定时任务失败: {e}")
            logger.error(traceback.format_exc())
    
    logger.info(f"当前平台：{platform}")
    # 弃用的弹幕客户端相关代码已清除，无需处理房间ID和弹幕客户端启动。
    if platform == "talk":
        thread.join()
    ...
    try:
        ...
        if my_handle is not None:
            try:
                ...
                logger.error(f"调用My_handle.shutdown失败: {e}")
        # 触发Audio内部关闭（通常由My_handle.shutdown包含）
        try:
            from utils.audio import Audio
        except Exception:
            pass
        logger.info("优雅关闭流程完成，准备退出。")
    except Exception as e:
        logger.error(f"退出处理异常: {e}")
        logger.error(traceback.format_exc())
    finally:
        # 使用os._exit确保尽快退出，避免卡在子线程；如果希望完全优雅可改用sys.exit
        import os
        os._exit(0)
