...
function save_config() {
    try {
        ...
        config["subtitle_bg_width"] = document.getElementById('input_subtitle_bg_width').value;
        config["subtitle_bg_height"] = document.getElementById('input_subtitle_bg_height').value;
...
function openDesktopWindow() {
    const url = `http://${getHostWithPortFromUrl(currentUrl).host}:${server_port}/open_desktop_window`;
    ...
    .then(data => {
        if (data.code === 200) {
            showtip("info", "独立桌面窗口启动成功");
        } else {
            showtip("error", "启动桌面窗口失败: " + data.message);
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showtip("error", "启动桌面窗口失败: " + error.toString());
    });
}
